...
indicator_data = {}
# Resample indicator data to weekly frequency and apply lag
for indicator in indicators:
    indicator_data[indicator] = pdr.get_data_fred(indicator, start=start_date, end=end_date)
    indicator_data[indicator] = indicator_data[indicator].shift(2)  # Lagging the indicator by 2 periods
    indicator_data[indicator] = indicator_data[indicator].resample('W').last()
...
for indicator in indicators:
    pivot_indicator_df[('fred', indicator, 'value-2')] = indicator_data[indicator]  # Adjust column name to indicate the shift
